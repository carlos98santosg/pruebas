...
int main(int argc, char* argv[]) {
    ...
    for (i=1; i <= num_processes; i++) {
    	...
    	//for(i=0; i<num_processes; i ++){
	    	if(fork()==0){
                //Imprime el fragmento que descargaremos
			    printf("Soy el hijo y descargo la parte %d.\n ",i);
			    printf("\t chunk #%d: Range %d-%d \n",i , from, to);

                //Genera el nombre del archivo que descargamos
                sprintf(outfile, "%s-%d", CHUNK_FILENAME_PREFIX, i);
           
                //Llamamos al método download_fragment
            ...
            //El padre esperará a que el hijo acabe su parte (En modo secuencial)
		    wait(NULL);
            }
	    }   
    ...
    if (download_mode == 'P') {
        //El padre no esperara a que los hijos descarguen su parte (En modo
        //paralelo)
